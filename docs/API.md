...
| `remote_build/get_identity` | — | `IdentityView` | Read the receiver's stable identity: `{dashboard_id, pin_sha256, server_version, esphome_version, listener_bound}`. The cert + key PEMs are intentionally NOT included; only the peer-link X25519 pubkey hash (`pin_sha256`, lowercase hex) is shipped. `listener_bound` reports whether the peer-link Noise WS listener is currently serving traffic. Idempotent (no rotation triggered). |
...
- `remote_build_pairing_window_changed` — `{open}` — fires when the pairing window opens / closes (refcount transitions, auto-close timeout). Subscribers update the "Pairing window: X seconds remaining" countdown.
