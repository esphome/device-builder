...
**Network providers (onboard ethernet).** A `featured_components` entry whose `component_id` is a network provider — `NETWORK_PROVIDER_COMPONENT_IDS` in `helpers/device_yaml/_generation.py`, currently `{"ethernet"}` — doubles as the board's onboard network. At `devices/create` with no `ssid`, `ComponentCatalog.resolve_network_components(board)` resolves that entry's locked pin presets and appends them to `generate_device_yaml`'s `defaults`. The generator detects a network provider in `defaults` and drops the `wifi:` block in its favour, emitting `api:` / `ota:` only when a network is actually present (Wi-Fi *or* an injected provider), so a wired-only board produces a compilable config without Wi-Fi secrets. Supplying an `ssid` skips the auto-pull and keeps Wi-Fi. The same entry still surfaces normally in the Add Component dialog's `featured` section, so it can also be added by hand on any board.
