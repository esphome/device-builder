...
Board catalog dataclasses (`BoardCatalogEntry`, `BoardHardware`, `BoardEsphomeConfig`, `BoardPin`, `FeaturedComponent`, `FeaturedBundle`, `DefaultComponent`, and the shared `FieldPreset`) use mashumaro's `omit_default = True` + `omit_none = True` Config, so any field whose runtime value equals the declared default or `None` is **absent from the wire payload** rather than being sent as `null` / `false` / `[]` / `""`. `from_dict` re-defaults missing fields from the same declared defaults, so the strip is lossless across the wire and the on-disk `definitions/boards.json` sidecar. The frontend hydrates at the API client (`hydrateBoard` in `src/util/board-hydrate.ts`) so consumer code reads the populated shape without per-call `?? []` defaulting. The shape descriptions below list every field; assume any field with a clear default-empty form (`null`, `false`, `[]`, `""`) is wire-omitted in that state.
