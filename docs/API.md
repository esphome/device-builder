...
| `list_pool` | offloader | Snapshot read merging in-memory PENDING pairings with persisted APPROVED rows. No wire calls. |
| `subscribe_pool` | offloader | Streaming command; thin `stream_events` drain over `offloader_pair_status_changed` events. Pure consumer — listener tasks are spawned at row creation in `request_pair`, not on subscription. |
...
| `remote_build/request_pair` | `{hostname, port, pin_sha256, receiver_label, offloader_label}` | `PairingSummary` | Re-handshake (defends against TOCTOU between preview and confirm), send `intent="pair_request"` carrying `{label: offloader_label, dashboard_id}` in encrypted msg3. APPROVED result (re-pair against existing trust): write the row to the offloader settings file as APPROVED. PENDING result: store the row in the controller's in-memory `_pending_pairings` dict (never on disk; a malicious LAN scanner can't bloat the offloader's settings file) and spawn a pair-status listener task that pushes `OFFLOADER_PAIR_STATUS_CHANGED` on the eventual flip. The PENDING dict doesn't survive a controller restart — any in-flight pair attempt has to be re-issued. `precondition_failed` on pin mismatch; `no_pairing_window` when the receiver's window is closed; `unavailable` on transport failure; `internal_error` on an unexpected receiver `intent_response`. |
...
| `remote_build/list_pool` | — | `[PairingSummary]` | Snapshot read merging in-memory PENDING pairings (`_pending_pairings`) with persisted APPROVED rows (offloader settings file). PENDING entries get `status="pending"` on the wire view; APPROVED get `status="approved"`. No wire calls to receivers — splits "first paint" cost from "live updates" so a slow / unreachable receiver doesn't block the Settings load. |
| `remote_build/subscribe_pool` | — | streaming | Long-running command; subscribes the calling WS client to `offloader_pair_status_changed` events. Thin `stream_events` drain — no listener-respawn logic, no presence refcount. The events come from the per-row pair-status listener tasks that were spawned when `request_pair` landed each PENDING row in `_pending_pairings`; the listeners run as long as the row stays PENDING regardless of whether anyone is currently subscribed. Frontend pattern: call `list_pool` once for the initial paint, then `subscribe_pool` for live updates. Parks until the WS closes. |
