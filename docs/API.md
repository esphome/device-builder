...
| `automations/parse` | `{configuration}` | `[ParsedAutomation]` | Walk the device YAML and return every recognised automation (top-level `script:` / `interval:`, `api.actions:`, device-level `esphome.on_*`, inline component `on_*:`, light `effects:` entries). An unknown action / condition id flags only its own automation (`error` set, empty tree); siblings still parse. A YAML that won't load at all raises `INVALID_ARGS`. |
...
| `remote_build/set_offloader_settings` | `{remote_builds_enabled?, version_match_policy?}` | `OffloaderRemoteBuildSettingsView` | Offloader-side: flip one or both master settings. `remote_builds_enabled=False` short-circuits `pick_build_path` to LOCAL. `version_match_policy` selects how strictly the scheduler filters peers by ESPHome version — `"any"` (default) skips the gate, `"release"` filters peers whose `YYYY.MM` differs from the offloader, `"exact"` filters any version diff with LOCAL fallback, `"exact_required"` raises `no_compatible_peer` when *any* APPROVED + enabled pairing exists and none are eligible (operator-visible signal — see issue #985). The strictest policy hard-fails on a transient peer-link disconnection just as readily as on a version mismatch, by design: silently routing to LOCAL behind an offline build server has the same surface as silently routing around a version-mismatched one. Either arg is optional; passing both `None` raises `invalid_args` to keep the surface loud about misuse. Unknown policy values also raise `invalid_args`. Each changed field fires its own event (`OFFLOADER_REMOTE_BUILDS_TOGGLED` / `OFFLOADER_VERSION_MATCH_POLICY_CHANGED`) for cross-tab sync and debounce-saves the pairings store. |
