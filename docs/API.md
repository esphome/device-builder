...
| `onboarding/set_wifi_credentials` | `{ssid, password}` | `OnboardingState` | Update `wifi_ssid` / `wifi_password` in `secrets.yaml` via a line-based rewrite that preserves comments and other secrets. Validates against ESPHome's own length limits (32 char SSID, 64 char password); empty SSID / oversize values raise `INVALID_ARGS`. Empty password is allowed (open networks). |
| `onboarding/mark_acknowledged` | — | `OnboardingState` | Record that the user has finished the current onboarding flow (sets `onboarding_completed_version` to `ONBOARDING_VERSION`). Idempotent. Use this on save AND on explicit decline ("I only use Ethernet") so the wizard stops re-popping; the per-step `pending` status stays accurate so the secrets-menu badge still reminds the user the underlying data isn't set. |
