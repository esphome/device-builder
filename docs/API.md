...
| `boards/get_board` | `{board_id}` | `BoardCatalogEntry` | Get one board's full body (hardware, pins, featured / default components). The response carries a derived `provides_network` flag (see network-providers note below) the create wizard reads to skip the Wi-Fi step for boards with their own network. |
