...
| `remote_build/unpair` | `{hostname, port}` | `{removed: bool}` | Pop the row from the unified `_pairings` dict and schedule the debounced save. Idempotent — `removed=false` when no row matched. Cancels the row's pair-status listener task. Auto-clears any pending offloader alert (`pin_mismatch` / `peer_revoked`) for the same `(hostname, port)`. The receiver-side `StoredPeer` is *not* notified; that's the receiver admin's concern (a future `intent="peer_link"` from this offloader will be rejected because the local row is gone). |
