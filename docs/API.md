...
| `artifacts_end` | receiver → offloader | `{job_id, accepted, reason?}` | Phase 6a. Stream terminator. `accepted=true`: assembler finalises (validates count + SHA-256), the parked `download_artifacts()` future resolves to `DownloadArtifactsResult(tarball, firmware_offset)`. `accepted=false`: carries a structured `reason` — `unknown_job`, `job_not_completed`, `duplicate_download`, `build_dir_missing`, `pack_failed`, `invalid_request` — the offloader's WS layer maps these to `not_found` / `precondition_failed` / `unavailable` / `invalid_args` `CommandError` codes. Tarball layout: `idedata.json` first (the upstream-canonical flash-image manifest, with `extra.flash_images[].path` carrying receiver-absolute paths the offloader rewrites to basenames at unpack time), then `firmware.bin`, then every `extra.flash_images` entry flattened to its basename. |
