...
| `no_compatible_peer` | Offloader's `version_match_policy="exact_required"` filtered every paired peer; install refuses to fall back to LOCAL (see `remote_build/set_offloader_settings`) |
