...
| `subscribe_pairings` | offloader | Streaming command; thin `stream_events` drain over `offloader_pair_status_changed` events. Pure consumer — listener tasks are spawned at row creation in `request_pair`, not on subscription. |
