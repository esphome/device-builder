...
| `.device-builder-cert.pem` | Public TLS cert (`dashboard_id` provenance only post-pivot; not used for transport security). Not sensitive. | mkstemp default (0o600) |
| `.device-builder-key.pem` | **Private TLS key. Sensitive.** Used to sign `dashboard_id` rotations. | 0o600 enforced at write time |
| `.device-builder-peer-link-key.bin` | **Private X25519 peer-link key. Sensitive.** A reader of this file can impersonate the dashboard to any paired peer over the Noise XX handshake. | 0o600 enforced at write time |
