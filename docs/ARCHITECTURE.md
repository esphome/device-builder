...
| checked | none | Fresh key minted — unless the resolved package itself ships `encryption:` (the running device may hold an NVS-provisioned key a competing baked key would break) or the resolve fails; both skip the mint and warn in the adopt dialog. An unresolvable package still gets a key when the unkeyed adoption warned inside the package and the keyed YAML validates clean |
