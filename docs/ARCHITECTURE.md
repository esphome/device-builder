...
**Persisted-IP revival** (`ApiReviverSource`, `controllers/_device_state_monitor/api_reviver.py`). The one Native API path that *does* drive state. A device whose mDNS responder goes quiet self-heals while the process stays up (the ping sweep keeps pinging its RAM `ip_addresses`), but after a restart RAM is empty, the `.local` won't resolve in a container, and the sweep claims OFFLINE with no target forever — even though the last-known IPv4 sits on disk (`Device.ip`, persisted for the OTA address cache and reseeded on cold load). A bare ICMP reply at that wall-clock-old DHCP address is inadmissible as ONLINE evidence (whatever now holds the lease answers — the #1776 latch class), so revival is identity-gated: candidates are `api:` devices with **no other reachability signal** (not ONLINE, persisted `ip`, empty RAM addresses, no zeroconf-cached addresses, a cached DNS failure proving the sweep already tried); the persisted IP is ICMP'd first as a purely *negative* filter (silence = no dial); only when something answers does one short-lived `device_info` worker dial run, and ONLINE is claimed — under the `ping` source, so the ordinary sweep owns liveness and demotion from then on — only when the reported name matches (MAC corroborating when both sides know it). A name mismatch instead proves the IP stale and clears it from the metadata store through `on_persisted_ip_invalidated`, the explicit counterpart to `on_ip_change`'s keep-last-known-on-disk contract. Because an API connect occupies one of the ESP's scarce connection slots, dials are serial, capped at 3 per sweep, back off exponentially on failure, and a process-lifetime verified `name → ip` cache caps the cost at one dial per stuck device per process (later flaps revive on the ICMP filter alone, the same trust RAM-learned addresses get). ICMP-unavailable deployments are deliberately not repaired: the negative filter can't run and a verify-only ONLINE could never demote.
