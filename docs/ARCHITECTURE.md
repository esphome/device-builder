...
device through the scanner's request/drain worker. Shallow rows are sticky
against the mtime cache — only a `reload` / `request` deepens them — which is
exactly what the refine provides; steady-state scans stay deep.
...
- **The refine emits `RELOADED`, never `UPDATED`.** `DEVICE_YAML_UPDATED`
  (version-history commits) keys on `UPDATED`, and the content-fingerprint
...
**Devices** (`_esphomelib._tcp.local.`) — passive browse. ESPHome devices broadcast on this service type; `DeviceStateMonitor`'s browser callback turns `Added` / `Updated` events into ONLINE claims and TXT-driven config-hash / version / api-encryption updates; a `Removed` withdraws the mDNS claim (state UNKNOWN, ledger released) and wakes the ICMP sweep to settle ONLINE/OFFLINE — deferring to a live sibling `_http._tcp` PTR that re-anchors the claim, and releasing only a claim mdns holds; with ICMP unavailable the withdrawal demotes straight to OFFLINE. See "Two mDNS paths with different OFFLINE semantics" in [CLAUDE.md](../CLAUDE.md) for the asymmetric trust rules between the browser callback and the one-off active-resolve path.

**Ping-target hygiene.** The sweep's targets stay trustworthy through two rules (#2486). A configured-address change (a `wifi.use_address` edit or removal, or a post-regen StorageJSON swap) runs `DeviceStateMonitor.address_retargeted`: unless mDNS/MQTT owns the name (their evidence is identity-carrying and address-independent), the ping-learned RAM `ip_addresses` are cleared before the sweep wakes, so the fallback can't keep pinging whatever answered at the old address and re-latch ONLINE. And unusable addresses — unspecified or loopback (`helpers/ip.py`) — are inadmissible liveness targets everywhere: the apply paths refuse them, a loopback DNS answer caches as a resolution failure (Pi-hole-style sinkholes), and `_resolve_and_ping` filters its final target list so `use_address: 127.0.0.1` applies OFFLINE instead of latching ONLINE off the dashboard host. The DNS cache still returns literal loopback verbatim, so SSH-tunnel OTA uploads keep working — only the ONLINE claim stops. Out-of-band YAML edits that change the config text (live as scanner UPDATED, or across a restart via the store-persisted `content_fingerprint` digest) schedule an `--only-generate` regen so `StorageJSON.address` and `expected_config_hash` track the YAML. The digest is version-prefixed; a stored value from an older digest algorithm re-seeds silently instead of scheduling a regen, so a dashboard upgrade can't spawn fleet-wide regens (#2612).

**HTTP identity fallback** (`_http._tcp.local.`) — passive browse on the same shared browser. A device without `api:` never publishes `_esphomelib._tcp` (that service is behind `USE_API`); its broadcast is the `_http._tcp` service — the bare fallback, or `web_server`'s own. On new firmware (esphome/esphome#17520) that service carries the identity TXT trio `version` / `mac` / `config_hash`; older firmware carries `version` only on the fallback and nothing on a web_server service. `MdnsSource._on_http_service_state_change` applies whichever identity keys are present for configured non-API devices through the shared `_apply_identity_txt` (plus the descriptive `project_name` / `project_version` / `network` keys through `_apply_descriptive_txt`, which deliberately never stamp freshness), so the card leaves "Waiting for mDNS discovery…" and the update dot can use the hash comparison. It deliberately never applies api-encryption (a device with no API has no encryption state to confirm) and drives **no** ONLINE claim off the HTTP service — its `Removed` is evidence-gated: it withdraws an mdns claim only when no live `_esphomelib._tcp` PTR remains to re-anchor it (an api+web_server bucket publishes both PTRs) and only for a name mdns owns; promotion stays owned by the active-resolve / MQTT / ping paths. It does own one non-reachability signal: the non-API side of `runtime_state.deployed_identity_live`, the session-only freshness bit the frontend gates the deployed identity on (a non-API device can hold `active_source == "mdns"` off a bare A-record resolve, but that vouches reachability only, so the api-device mdns gate deliberately doesn't apply). Every identity-bearing apply stamps it true, as does the post-flash optimistic sync (first-party evidence for mDNS-dark deployments); the API-info sweep's level-sync stage below owns the non-API clear side. The same flag also carries the api-device Native-API evidence — see the fallback below. See "Two mDNS paths with different OFFLINE semantics" in [CLAUDE.md](../CLAUDE.md).

**Native API info fallback** (`ApiInfoSource`, `controllers/_device_state_monitor/api_info.py`). When mDNS multicast can't reach the dashboard (the common Docker-bridge case) a device is ONLINE via ping but its `mac_address` / `deployed_version` stay blank — those come only from the `_esphomelib._tcp` TXT records. A fourth source repairs them in three stages per sweep. First a free, level-triggered reconcile: re-apply the zeroconf-*cached* TXT payload (both the `_esphomelib._tcp` record and a non-API device's `_http._tcp` identity TXT) for any online device still missing a field (no claim, no IP — a cache hit can be stale). The browser's apply path is edge-triggered, so an announce whose resolve timed out leaves the record blank while its records still land in the cache, and zeroconf never re-fires the handler for same-content TTL refreshes (#1910). Second, another free stage level-syncs every non-API device's `deployed_identity_live` flag against the unexpired cached `_http._tcp` identity TXT: a live TXT under a lowered flag re-runs `reconcile_from_cache` (which also refreshes *populated* fields the missing-field gate never revisits), and a raised flag over a dark cache triggers the targeted `verify_http_identity` re-resolve (capped per sweep like the API probes), clearing only on a confirmed miss — and only for devices with some cached mDNS trace, since an mDNS-dark deployment's wire miss proves nothing and its post-flash stamp must survive. Third, for devices the cache can't fill, a Native API connect. That stage is deliberately narrow: one probe at a time, only for an online device that loads the `api` integration, still misses a field (regardless of which source owns it — ownership proves a resolve, not an applied TXT), is off its failure cooldown, and has a routable IP. `aioesphomeapi` never loads into the long-lived process — a `find_spec` guard plus a short-lived `python -m esphome_device_builder.helpers.api_device_info` worker keep it in the child (the encryption key is passed over stdin, never argv). It only ever writes TXT-derived fields through the `apply_*` methods, so it **stays out of the `mdns > mqtt > ping` precedence ledger** and never drives ONLINE/OFFLINE. A payload that carried mac or version also stamps `deployed_identity_live` (a confirming re-probe counts — the connection itself is the evidence); the stamp site is `apply_worker_info`, shared with the API reviver's verified revival dial. The ownership rule lives once, in the monitor: a `live=True` stamp is refused while mDNS owns an api device, the flag clears when mDNS takes ownership (the announce lifecycle vouches from there), and a withdrawal stamps it back for an api bucket with known identity — vouching follows ownership in both directions. An mDNS-dark api device therefore keeps its Native-API-confirmed identity trusted the same way an mDNS-dark non-API device keeps its post-flash stamp.

**Persisted-IP revival** (`ApiReviverSource`, `controllers/_device_state_monitor/api_reviver.py`). The one Native API path that *does* drive state. When a device's mDNS responder goes quiet, the `.local` won't resolve in a container, and its RAM `ip_addresses` are gone (cleared when a `Removed` withdraws the mDNS claim, or empty after a restart), the sweep claims OFFLINE with no target forever — even though the last-known IPv4 survives in `Device.ip` (RAM mirrors the metadata sidecar; a `Removed` clears only the resolved set, #2029). A bare ICMP reply at that wall-clock-old DHCP address is inadmissible as ONLINE evidence (whatever now holds the lease answers — the #1776 latch class), so revival is identity-gated: candidates are `api:` devices with **no other reachability signal** (not ONLINE, persisted `ip`, empty RAM addresses, no zeroconf-cached addresses, a cached DNS failure proving the sweep already tried); the persisted IP is ICMP'd first as a purely *negative* filter (silence = no dial); only when something answers does one short-lived `device_info` worker dial run, and ONLINE is claimed — under the `ping` source, so the ordinary sweep owns liveness and demotion from then on — only when the reported name matches (MAC corroborating when both sides know it). A name mismatch instead proves the IP stale and clears it through `on_persisted_ip_invalidated`, the one path that drops a last-known `Device.ip` (RAM and metadata store). Because an API connect occupies one of the ESP's scarce connection slots, dials are serial, capped at 3 per sweep, back off exponentially on failure, and a verified `name → ip` cache lets flaps revive on the ICMP filter alone (the same trust RAM-learned addresses get) but only within a freshness TTL — past it one dial re-verifies, so a lease reassigned during a long silent gap can't ride a stale verification back to ONLINE. ICMP-unavailable deployments are deliberately not repaired: the negative filter can't run and a verify-only ONLINE could never demote.

**MQTT** (`DeviceMqttCoordinator` / `DeviceMqttMonitor`, `controllers/_device_mqtt_*.py`): opt-in per device via its `mqtt:` block. The coordinator resolves each MQTT-using device's broker fields (`!secret` plus local substitutions), groups devices by broker identity, and runs one paho session per unique login; devices answer subscriber-gated `esphome/discover` broadcasts sent by one elected broadcaster per broker. TLS engages when the block carries a `certificate_authority` (inline PEM, the shape esphome expects): the monitor builds an `ssl.SSLContext` inside the executor hop (SSLContext loading does hidden blocking I/O) and verifies the chain, while `skip_cert_cn_check` maps to paho's `tls_insecure_set`, never a direct `check_hostname` flip (paho 1.6.1 would then call `ssl.match_hostname`, removed in Python 3.12). The CA rides the broker key as a digest so a rotated cert replaces the running session on the next reconcile. A declared CA that is missing, unresolvable, or not loadable PEM refuses the broker with a gated warning rather than degrading to plaintext, and client-certificate blocks are skipped with a warn-once since the stdlib loads cert chains only from files. Broker resolution is tiered by freshness: the scanner carries each mqtt device's tolerant-parsed main-file block (`!secret` markers unresolved) plus the package-merged block on `Device.mqtt_extract` (serialize-omit, never on the wire); reconcile consumes the main-file block while its recorded `(mtime_ns, size)` matches the file, re-reading on mismatch, and the package-merged seed is the only source for package-sourced values — the coordinator never runs the esphome parse itself; an absent, stale, or shallow seed becomes a queued scanner reload (see "Cold-start device scan"). Secrets stay per-poll — resolution always runs against the current `secrets.yaml`, so a secrets repair recovers on the next reconcile with no YAML touch.

**Dashboards** (`_esphomebuilder._tcp.local.`) — bidirectional. The dashboard advertises its own service instance on startup, HA-addon mode included (the addon runs host-network, so its addresses are LAN-routable; the advertise is discovery-only and doesn't imply a peer-link receiver is bound). The service-instance name and SRV target are stable per-install identifiers (`esphome-builder-<dashboard_id[:8]>`) derived only from the persisted `dashboard_id`, never the OS hostname; this keeps the SRV target from flipping when the OS hostname changes across reboots (macOS flips `mac` ↔ `macbook-pro`), which otherwise forced a needless endpoint rebind on every restart. TXT carries `server_version` + `esphome_version` + `friendly_name` (the human machine label, carried here because the instance name is now an opaque identifier) always; `pin_sha256` + `remote_build_port` are added when the remote-build receiver site is bound. Browse runs in `RemoteBuildController` and populates `self._peers`; a sync `hosts_snapshot()` seeds the `subscribe_events` initial-state push under `hosts` and the browser's `_on_service_state_change` / `_resolve_and_apply` callbacks fire `remote_build_host_added` / `remote_build_host_removed` events as dashboards come and go. Cross-subnet peers (the LAN's mDNS doesn't reach them) bypass discovery entirely — the pair dialog accepts a typed `hostname` / `port` and `request_pair` either succeeds or fails.

**Post-bind advertise invariant.** The advertise and the peer browse run in one background task chained off `DeviceBuilder.start()`, gated on `notify_serving()` — released by `web.run_app`'s post-bind `print` hook (the only aiohttp signal that fires after every site's socket is accepting), or right after the peer-link bind check in `--remote-build-only` mode. The gate means the announce can never point peers at a dashboard that isn't serving yet, and the browse always starts after the register so `own_instance_name` reflects any `allow_name_change` rename. A new serving path must open the gate (route it through `DeviceBuilder._run_web_app`, which owns the hook); a path that forgets logs a warning after a bounded wait and advertises anyway.
...
**Design scope: LAN-class links.** The peer link assumes LAN-class latency and bandwidth end to end — session keepalive deadlines, transfer pacing, and the deliberate absence of link-speed tuning knobs all bake in that assumption. Operation across a VPN or WAN (Tailscale, cross-site) is best effort: defects that reproduce at any link speed are in scope, but tuning for or accommodating slow links is not, and issues that only reproduce over a slow link will generally be closed pointing here. The manual-entry pairing path exists for routed LANs and mDNS-reflector setups, not to endorse WAN deployment.
...
5. **Pair request (intent=pair_request)** — fresh Noise XX with payload `{label, dashboard_id, friendly_name, ha_addon, label_auto}` plus `pairing_key` when the receiver's key-gated bootstrap window is armed. If the pairing window is open and no APPROVED row exists yet, the receiver adds a PENDING entry to its in-memory `_pending_peers` dict (no disk write), fires `remote_build_pair_request_received`, and returns `intent_response=pending`. If the window is closed, returns `intent_response=no_pairing_window`. If an APPROVED row already exists with a matching pin, the receiver refreshes the row's introduction fields from the request (debounced save + `remote_build_peer_refreshed` when any field changed; a no-op retry costs nothing) and returns `intent_response=approved` immediately (re-pair against existing trust, bypasses window gate).
...
**Second TCP listener.** When `_remote_build.enabled` is `true` — or, connect-back only, when the offloader has at least one APPROVED pairing — `DeviceBuilder` binds an aiohttp site on `--remote-build-port` (default 6055) serving `/remote-build/peer-link`. A taken port falls forward to the next free one (bounded scan of `REMOTE_BUILD_PORT_SCAN_ATTEMPTS` candidates, each held on every bind host from probe to listen so concurrent starters can't race it) — the stable/beta/dev add-on flavors share one host-network host, and only one can hold each port. Default is `True` on standalone / Desktop installs; on the HA addon a fresh install (no persisted `_remote_build` block) reads as `False` — the addon runs host-network, so a default bind would silently open a new LAN-reachable listener, and the operator opts in via the Settings toggle instead. The rule lives in `effective_remote_build_settings`, shared by the bind site and the receiver's settings surface so the toggle the frontend shows always matches what the bind site did. When the toggle is off and no approved pairing exists the listener doesn't bind at all (a sidecar `enabled=false` skip beats default-deny 404s — nothing to probe). This sits alongside the public + ingress sites from the Authentication section: HA-addon mode with remote-build enabled binds three listeners on three different ports, each with its own role.

**Connect-back-only bind.** A dashboard with the Build-server toggle off but at least one APPROVED pairing binds the same listener in a reduced role so a paired build server can dial back (see *Receiver connect-back* under Endpoint rebind): every receiver intent (`preview` / `pair_request` / `peer_link` / `pair_status`) is answered `REJECTED/bad_intent` — indistinguishable from an old listener, so the mode never silently re-enables pairability — and the mDNS TXT advertises neither `pin_sha256` nor `remote_build_port`, so the dashboard doesn't appear as a pairable build server in discovery. This applies on the HA add-on too — the connect-back listener is on by default for any add-on offloader with pairings, independent of the add-on's default-off Build-server toggle. There is deliberately no separate toggle: an approved pairing *is* the opt-in and unpairing the opt-out — the feature exists to self-heal without the operator ever discovering it, and the bind exposes only the Noise XX surface. `RemoteBuildLifecycle.converge()` recomputes the policy on every settings write and approved-pairing transition; a role flip tears down and rebuilds so the intent gate and TXT match. `IdentityView.listener_bound` can therefore be true while `enabled` is false.
...
The TXT contract — `pin_sha256` + `remote_build_port` appear together iff the listener is currently bound *in receiver role* (a connect-back-only bind advertises neither) — holds across rotation. When the listener isn't bound, rotation only writes new keys to disk; mDNS isn't updated because there's no listener for peers to connect to.
...
* `download_artifacts` → `ArtifactsDownloadSender` reads `idedata.json` + flash images via the shared `helpers/build_artifacts.py` discovery helper, packs them into a gzipped tarball off the event loop, and streams the bytes back as `artifacts_start` → `artifacts_chunk` → `artifacts_end` from a tracked task off the receive loop (a slow transfer blocks on TCP backpressure for its whole wall clock; inline it starved the loop of pings and pongs until the heartbeat killed the session, #2377). Validation and the single-flight `duplicate_download` gate stay inline; an off-loop crash sends a terminal `artifacts_end{accepted: false, reason: stream_failed}` so the offloader is never stranded. `firmware_offset` rides on the start frame so the offloader doesn't duplicate platform-detection logic.
...
**Heartbeat.** Symmetric, encrypted, both directions: each side sends `{"type": "ping", "nonce": N}` every `HEARTBEAT_INTERVAL_SECONDS` and treats **any successfully decrypted inbound frame** as liveness — the ping/pong pair only guarantees traffic on a quiet link. Going `HEARTBEAT_DEAD_AFTER_SECONDS` without an authenticated frame closes the session — receiver via `terminate{reason: heartbeat_timeout}`, offloader via WS close + the offloader's `_run_session_loops` shared-state surface that propagates `heartbeat_timeout` into the local close reason instead of falling through to the default `peer_hung_up`. Liveness is deliberately not pong-only: all frames share one WS and one send lock, so a peer streaming a large artifact tarball over a slow link can hold its pong behind minutes of queued chunk frames — the chunks themselves are the proof of life (#2377).
...
**Endpoint rebind.** A paired receiver that changes hostname / port stays paired — same `pin_sha256`, different routing coordinates. Three entry points share one commit primitive:
...
* **Receiver connect-back.** The automatic cross-subnet repair. Each forward `peer_link` msg3 carries the offloader's bound listener port as `connect_back_port`; the receiver persists it on the `StoredPeer` along with the session's source IP (both refreshed on every session-open, non-zero / non-empty only, so an older offloader never clobbers a captured value). When a peer has had no inbound session for `_CONNECT_BACK_QUIET_SECONDS` (5 min — well above the forward client's 30s reconnect cap, so a reachable offloader always reconnects first and cross-dials stay rare), the receiver's sweep (`controllers/remote_build/connect_back.py`, 30s cadence) dials that last-known endpoint with `intent="connect_back"`, Noise-pinned to the stored offloader key, announcing its own listener port as `peer_link_port`. The offloader treats the reverse connection's *source IP* + announced port as the candidate — never a claimed address — and runs the same probe + commit primitives (`rebind.handle_connect_back`); nothing is persisted unless the forward probe verifies the pin at that endpoint. Replies map onto the receiver's per-peer backoff (jittered exponential, 5 min base → 1 h cap): `ok` resets it, `rejected/rebind_in_progress` retries shortly, `rejected/already_connected` / `probe_failed` escalate, `bad_intent` (an older offloader) parks at the cap. Exactly one connection survives every race: a live forward link refuses the announce (`already_connected`), an in-flight mDNS probe holds the shared per-pin cooldown slot (`rebind_probe_until`), the commit awaits the old client's teardown before respawning, and the receiver cancels its dial the moment an inbound session registers. The dial itself only runs while the listener is bound in receiver role — the announce is our own peer-link port.
...
The lock keys on `data_dir`, not `config_dir`, so the HA addon's `esphome` / `esphome-beta` / `esphome-dev` flavors — distinct per-slug `/data` mounts but a shared `/config/esphome` YAML tree — can run in parallel. The `config_dir`-resident state that's still shared across flavors is handled separately: `.device-builder.json` takes an `fcntl.flock` inside `helpers/metadata_sidecar.metadata_transaction` so cross-flavor RMW writes can't clobber each other; the peer-link key and `dashboard_id` are first-write-only / idempotent after creation. The per-device *live* state (running-firmware hash, observed version, etc.) sidesteps the cross-flavor problem entirely by living in `<data_dir>/.device-builder-devices.json` instead — see *Per-device metadata split* above. The deployment-modes table at the top of `CLAUDE.md` is load-bearing here — `CORE.data_dir` resolves to `/data` (HA addon), `$ESPHOME_DATA_DIR` (env override), or `<config_dir>/.esphome` (default), and the lock-key choice rides on that.
...
**On Windows the build tree is relocated to a short, space-free root.** Native Windows ESP-IDF builds fail two ways from a normal config path: the deep build tree overflows the 260-char `MAX_PATH`, and a space in the profile/config path (`C:\Users\First Last\…` is common) trips pioarduino's `FRAMEWORK_DIR`/`BUILD_DIR` whitespace guard (older platforms truncate gcc `-fdebug-prefix-map` at the space). At startup `helpers/windows_build_paths.windows_short_build_paths` sets `ESPHOME_DATA_DIR = C:\esphb\<dashboard_id[:8]>`, `PLATFORMIO_CORE_DIR = <root>\pio` and `ESPHOME_ESP_IDF_PREFIX = C:\esphb\idf` — short, space-free **real** directories (not a junction: CMake `REALPATH`s a junction back to its spaced/long target). The native ESP-IDF toolchain is the one piece **shared across dashboards** rather than per-root: gcc probes its multilib include dirs via un-normalized `bin/../lib/...` self-relative paths reaching ~245 characters below the IDF tools dir, and Windows checks the string as given, before collapsing `..` (esphome/esphome#16896) — so the prefix must stay within 15 characters of `MAX_PATH`; `C:\esphb\<id8>\idf` (21) overflows while `C:\esphb\idf` (12) fits, and sharing matches upstream's machine-global cache semantics (the tools dir is per-version inside). The shared dir is never seeded from an existing tree; the toolchain installs into it fresh — penv paths bake into every configured build dir, so a *moved* IDF tree fails idf.py's active-vs-configured python check — an earlier release's per-dashboard `<root>\idf` is deleted rather than swept in, and esphome's machine-global cache is left untouched for native CLI use. Per-dashboard roots nest under one `C:\esphb` parent (the first relocation release used a flat `C:\esphb-<id8>`; the build now moves that legacy layout into `C:\esphb\<id8>` once via an atomic same-volume rename, falling back to the flat root if the move fails). This is just the `ESPHOME_DATA_DIR` override row of the deployment-modes table, so `CORE.data_dir` resolves there for the dashboard's own reads *and* every compile subprocess with no divergence. Existing data is moved into the root once so warm caches survive: from the legacy flat root, else from `<config_dir>/.esphome` + `~/.platformio`. The tree lives outside `<config_dir>` and is **left on uninstall** — a reinstall keeps the warm toolchain, and a user can delete `C:\esphb` by hand to reclaim the space (documented in the esphome-desktop README). The gate is `os.name == "nt"`, so it is a **no-op inside a Linux Docker container on Windows** (the common containerized shape, and the HA addon) — that process is posix and keeps its normal data dir; relocation only applies to native-Windows Python (ESPHome Desktop / Windows pip). No-op on every non-Windows platform.

**Windows startup also suppresses hard-error dialogs for the whole build chain.** `helpers/windows_error_mode.suppress_child_error_dialogs` (called once in `__main__.main()`, a no-op off Windows) ORs `SEM_FAILCRITICALERRORS | SEM_NOGPFAULTERRORBOX | SEM_NOOPENFILEERRORBOX` into the process error mode. Children inherit the error mode, so every compile/flash descendant down to the toolchain binaries runs with modal error dialogs disabled; without it, binutils `ar.exe` raises a "Bad Image" dialog per invocation when a toolchain ships a non-DLL in `lib/bfd-plugins/` (binutils bug 27113; `libdep.a` in the RP2040 toolchain, #2562), and a background build service must never block its children on a dialog.
