...
| `.device-builder-sessions.json` | `<config_dir>` | **Opaque dashboard session tokens. Sensitive.** A reader can replay a token as an authenticated WebSocket client until it expires (30 days sliding). Owned by `SessionStore` (`helpers/auth.py`); expired rows are dropped at load. | 0o600 enforced at write time (default for `Store`); debounced, flushed at graceful stop |
