...
**Why long-poll instead of frontend polling.** The original design polled `list_pairings` every 5s while a pending row was visible; the current design holds a Noise WS open with `intent=pair_status` for each PENDING row. The receiver-side `lookup_peer_for_status` parks on its own bus's `pair_status_changed` event filtered to the matching `dashboard_id` and pushes the response when admin clicks Accept / Reject — sub-second flip latency without a poll cadence. Transport errors retry after a 2s backoff; terminal flips (APPROVED / REJECTED) exit the listener.

**PENDING is in-memory only, bounded by the pairing window.** Disk only carries APPROVED rows. PENDING entries live in `RemoteBuildController._pending_peers: dict[str, StoredPeer]` (receiver side) and `_pending_pairings: dict[tuple[str, int], StoredPairing]` (offloader side) for the *active pairing window's* lifetime; the dict is cleared on every window-close transition (auto-close timeout, explicit `set_pairing_window(open=False)`, controller `stop()`). The clear path fires `pair_status_changed("removed")` for each cleared entry so any in-flight pair_status long-poll wakes, re-snapshots, and reports REJECTED to its offloader; the offloader's listener then drops its own pending state. Three load-bearing properties fall out of this:
...
3. **Cold-start has no PENDING state** — a controller restart means the dict starts empty; any in-flight pair attempts have to be re-initiated by the offloader. The offloader-side mirrors this: `subscribe_pairings` is a thin `stream_events` drain with no respawn-on-entry logic, because a fresh dict has nothing dormant to respawn.
