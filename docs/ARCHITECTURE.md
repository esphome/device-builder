...
**Remote build-env reset.** A paired offloader can run the receiver's full local `firmware/reset_build_env` (`esphome clean-all` + every cached venv — the last-resort hammer for a wedged toolchain on a headless build server) as a tracked job: `remote_build/reset_peer_build_env` enqueues a REMOTE-source RESET_BUILD_ENV *mirror job* on the offloader, whose runner sends one bundle-less `reset_build_env{job_id}` frame; the receiver enqueues its own local reset job tagged `remote_peer` + `remote_job_id`, so the 2–15 minute wipe streams `job_output` / `job_state_changed` back through the existing `JobFanout` into the mirror job, and `cancel_job` works through the same correlation. Capability-gated (`reset_build_env_supported` on the intent_response; old receivers never advertise it). The receiver refuses `busy` while *any* job is active or a bundle is mid-upload — a full reset wipes the shared toolchain every tenant compiles from, and a peer must never force-cancel other dashboards' work (deliberately *not* the local command's cancel-all; the handler replicates only `_disarm_all_queued_updates` + tagged create + enqueue). The receiver-side job is otherwise the stock local reset: compile-lane, `upload_blocked` holds its own flashes, `_clean_provisioned_venvs` runs on success. On the offloader, a REMOTE-source reset is exempt from `upload_blocked` (it wipes the receiver's tree, not any local artifact a flash reads). A peer-link loss mid-reset fails the mirror job with `session lost`, but the receiver's wipe runs to completion independently — the reset is idempotent, so a retry when the link returns just finds less to delete.
