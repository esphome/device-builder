...
**Remote build-env reset.** A paired offloader can wipe its own isolated subtree on the receiver (`remote_build/reset_peer_build_env` → `reset_build_env` frame → ack): the receiver derives the target from the session's authenticated `dashboard_id`, refuses `busy` while that offloader has a job queued / running or a bundle mid-upload, and `rmtree`s both per-offloader trees (`<config_dir>/.esphome/.remote_builds/<dir_id>/` + `<data_dir>/.remote_builds/<dir_id>/`) in an executor — never the shared `venvs/` cache, so one peer's reset can't break another's in-flight build. Capability-gated (`reset_build_env_supported` on the intent_response); old receivers never advertise it and old offloaders never send the frame. The receive loop awaits the handler, so no same-session submit interleaves with the wipe; a concurrent TTL-sweep delete degrades to a logged warning (every sweep delete is OSError-guarded).
