...
**HTTP version fallback** (`_http._tcp.local.`) — passive browse on the same shared browser. A device with `mqtt:` but no `api:` never publishes `_esphomelib._tcp` (that service is behind `USE_API`); its only broadcast is a bare `_http._tcp` fallback carrying a lone `version` TXT. `MdnsSource._on_http_service_state_change` reads that `version` for configured non-API devices and feeds it through the existing `apply_version` path, so the card leaves "Waiting for mDNS discovery…". It is deliberately version-only: `mac` / `config_hash` / api-encryption live only on `_esphomelib._tcp`, and it drives **no** ONLINE/OFFLINE state (no claim off the HTTP service) — reachability stays owned by the active-resolve / MQTT / ping paths. See "Two mDNS paths with different OFFLINE semantics" in [CLAUDE.md](../CLAUDE.md).

**Native API info fallback** (`ApiInfoSource`, `controllers/_device_state_monitor/api_info.py`). When mDNS multicast can't reach the dashboard (the common Docker-bridge case) a device is ONLINE via ping but its `mac_address` / `deployed_version` stay blank — those come only from the `_esphomelib._tcp` TXT records. A fourth source connects over the Native API to fill them. It is deliberately narrow: one probe at a time, only for an online device that loads the `api` integration, is *not* owned by the mDNS source (so mDNS clearly isn't delivering), still misses a field, is off its failure cooldown, and has a routable IP. `aioesphomeapi` never loads into the long-lived process — a `find_spec` guard plus a short-lived `python -m esphome_device_builder.helpers.api_device_info` worker keep it in the child (the encryption key is passed over stdin, never argv). It only ever writes mac/version through `apply_mac_address` / `apply_version`, so it **stays out of the `mdns > mqtt > ping` precedence ledger** and never drives ONLINE/OFFLINE.
...
The trust model matches Bluetooth-style pairing: the window is explicit (operator started the process), time-boxed, single-use, and the OOB fingerprint compare on the main builder is the load-bearing verification. A LAN attacker racing the window would have to win the first request *and* survive the operator seeing their own pair attempt fail with a pin mismatch. The pairing-source posture is never a silent default: `--remote-build-only` **requires** either `--allow-pairing-source <IP>` (comma-separated, IPv4/IPv6) or an explicit `--allow-any-pairing-source`, and the parser rejects both-at-once and passing either without `--remote-build-only`. With `--allow-pairing-source`, the bootstrap auto-approves only a `pair_request` whose peer IP is on the list — `pair_flow._pairing_source_allowed` gates the auto-approve, and a non-matching source is refused as a closed window (leaking nothing) *without* disarming, so the intended builder can still pair; this drops the "race the window" vector for an operator who knows the main builder's address ahead of time. `--allow-any-pairing-source` is the conscious opt-in to trust-on-first-use (the banner says so in as many words); the accepted residual risk is written up in `docs/THREAT_MODEL.md`. The emoji rendering is the Matrix SAS spec's 64-emoji vocabulary (6-bit chunks of the leading 42 bits) — `helpers/pin_emoji.py` is an exact port of the frontend's `src/util/pin-emoji.ts`, and each side pins the algorithm with a shared known-vector test so the CLI banner and the pair dialog always show the same sequence for a given pin.
