...
@_tool(
    ...
    "false means the upload job could not be read: poll the compile job, do not install "
    "again. A flash has no undo: the device runs whatever compiles, so read and validate the "
    "config first.",
    {
        "configuration": _CONFIGURATION,
...
@_tool(
    ...
    "list_automations). automation is {trigger_id, trigger_params, actions} where trigger_id "
    "is the qualified id from get_available_automations such as binary_sensor.on_press and "
