...
class JsonRpcErrorCode(IntEnum):
    """The JSON-RPC 2.0 error codes this server emits."""
...
class McpServer[ContextT]:
    ...
    async def _tools_call(self, context: ContextT, params: dict[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(name, str) or name not in self._tools:
            raise _RpcError(JsonRpcErrorCode.INVALID_PARAMS, f"Unknown tool: {name}")
...
def _object_param(container: dict[str, Any], key: str) -> dict[str, Any]:
    """Read the object at *key*; omitted is ``{}``, any other non-object is INVALID_PARAMS."""
    if key not in container:
        return {}
    value = container[key]
    if not isinstance(value, dict):
        raise _RpcError(JsonRpcErrorCode.INVALID_PARAMS, f"{key} must be an object")
    return value


def _error_response(msg_id: Any, code: JsonRpcErrorCode, message: str) -> web.Response:
