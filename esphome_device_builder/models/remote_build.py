...
class StoredPeer(DataClassORJSONMixin):
    ...
    pair-request flow over the peer-link WS: an offloader sends
    a Noise XX handshake with ``intent="pair_request"`` and the
    payload carries its ``cert_pem`` and a label; receiver
    derives the static X25519 pubkey from the cert (Ed25519 →
    X25519 conversion, RFC 7748 §5), stores both, and pins on
    the X25519 pubkey for every subsequent connection.
    ...
    fields where we already have a hex-pin convention from
    receiver-cert pinning.

    ``cert_pem`` is the offloader's full self-signed cert
    (Ed25519 keypair, generated by their phase-3a identity
    helper). Kept around so a cert-rotation event can re-derive
    the X25519 pubkey from the new cert and verify it against
    the stored one — diff = pin_mismatch event, prompts re-pair.

    ``dashboard_id`` is the offloader's stable identity from
    phase 3a; sent in the pair_request payload so the admin UI
    has a friendly identifier (cert + X25519 pubkey alone don't
    carry one). Primary key for the receiver WS surface
    (``approve_peer({dashboard_id})`` etc.) so a future cert
    rotation on the offloader's side doesn't change the user-
    facing handle.
...
class PeerSummary(DataClassORJSONMixin):
    ...
    Drops ``static_x25519_pub`` (the raw bytes; ``pin_sha256``
    is already the wire-friendly form) and ``cert_pem`` (the
    full PEM is on-disk-only; the SPKI hash is what UIs render
