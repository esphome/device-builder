...
def build_board_catalog_from_manifests(*, strict: bool = False) -> BoardCatalogResponse:
    ...
    Used by ``script/sync_boards.py`` to regenerate
    ``definitions/boards.json``. The runtime loader is
    ``load_board_catalog`` — it reads the prebuilt JSON.

    When ``strict`` is true, any per-board failure aborts the whole
    walk by re-raising the exception (sync-script behaviour). When
    false, failures are logged and the broken board is skipped
    (preserves the historical runtime fallback semantics).
    """
...
def load_board_catalog() -> BoardCatalogResponse:
    ...
    The JSON is generated by ``script/sync_boards.py`` from the
    YAML manifests under ``definitions/boards/`` and ships inside the
    wheel. Decoding it via ``orjson`` + mashumaro is roughly two orders
    of magnitude faster than re-parsing every manifest at startup —
    critical for low-powered hosts like HA Green where the YAML walk
    used to dominate dashboard startup time.

    When the JSON is missing, returns an empty catalog and logs a
    warning (mirrors ``ComponentCatalog.load``); source installs need
    to run ``script/sync_boards.py`` once before startup.
    """
