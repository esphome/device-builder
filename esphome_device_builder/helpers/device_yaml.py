...
# Sentinel distinguishing "caller passed no resolved_config" from a
# legitimate ``None`` (parse failure) so the detector only loads the
# config itself when the caller didn't already resolve it.
_UNSET: Any = object()
...
def detect_platform_from_yaml(
    ...
    *,
    ...
    Callers on the hot scan path (``load_device_from_storage``) pass
    the already-read *yaml_content* and already-merged
    *resolved_config* so the detector reuses them instead of a
    second ``read_text`` + ``load_device_yaml`` (which can re-clone
    remote packages).
