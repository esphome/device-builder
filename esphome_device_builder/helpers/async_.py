...
Ported from home-assistant/core ``homeassistant.util.async_``.
"""
...
def create_eager_task[T](
    ...
    *,
    ...
    suspension point, so a coroutine that completes without ever awaiting
    never reaches the event loop's task queue.
    """
    if loop is None:
