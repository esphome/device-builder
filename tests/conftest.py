...
def make_remote_build_controller(
    *,
    ...
    # Explicit miss, not an auto-mock: cancel_job's local-first branch probes
    # ``firmware.get_job`` and a MagicMock would claim every job id.
    db.firmware.get_job = AsyncMock(return_value=None)
    if bus is not None:
        db.bus = bus
...
def reset_offloader_firmware_stub(
    ...
    **queue_status_kwargs: Any,
    ...
    firmware.get_job = AsyncMock(return_value=None)
    return firmware
