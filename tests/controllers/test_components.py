...
async def test_helper_aliases_come_from_the_capabilities_snapshot(
    ...
    monkeypatch.setattr(
        ...
        lambda: EMPTY_PLATFORM_CAPABILITIES._replace(component_names=["esp32_ble_client"]),
    )
    docs = await cat.get_integration_docs()
...
async def test_get_component_bodies_resolves_rp2040_alias() -> None:
    """A ``rp2040`` request hydrates the canonical ``rp2`` body, keyed as requested."""
    cat = ComponentCatalog()
...
def test_index_title_resolves_rp2040_alias() -> None:
    cat = ComponentCatalog()
