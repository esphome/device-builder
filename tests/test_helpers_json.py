...
self-evident from a glance, but ``dumps_str_subclass_keys`` flips
...
def test_dumps_rejects_str_subclass_keys() -> None:
    ...
    endpoint reaches for ``dumps_str_subclass_keys`` precisely
...
def test_dumps_str_subclass_keys_serialises_str_subclass_keys() -> None:
    """``dumps_str_subclass_keys`` permits ``str``-subclass keys.
    ...
    out = dumps_str_subclass_keys(payload)
    assert isinstance(out, str)
    assert loads(out) == {"esphome": {"name": "kitchen"}}


def test_dumps_str_subclass_keys_serialises_plain_str_keys() -> None:
    ...
    common case.
    """
    out = dumps_str_subclass_keys({"plain": 1, "nested": {"x": 2}})
    assert isinstance(out, str)
