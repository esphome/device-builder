...
def test_first_creation_is_serialised_under_lock(tmp_path: Path) -> None:
    """
    Two callers racing on a fresh dir must not both write a key.

    The lock + load-after-take pattern is load-bearing: without it,
    the loser would silently invalidate the winner's freshly
    generated bytes (different keypairs land on disk depending on
    interleaving). Verify by counting writes through a tracking
    wrapper across three sequential calls — only the first writes.
    """
    ...
    # Three calls but only the first creates the file.
