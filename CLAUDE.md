...
- **esphome owns config validation; never pre-validate fields against
  ...
  bounded run `upsert_automation` appends to its reply),
...
| `esphome_device_builder/definitions/migration_rules.index.json` | Generated; do not hand-edit. Direct `cv.rename_key` pairs and component-ALIAS key renames the sync discovered, each with its `since` / `removed_in` esphome window, folded as version-gated data-driven rules by `editor/migrate_config`. The artifact itself is backend-internal; the rules that fired reach the frontend as `changes` rows on the migrate reply. Loaded via `load_migration_rules_index`. |
