#!/usr/bin/env python3
"""Regression check for `import esphome_device_builder.device_builder` cost.

Runs `python -m importtime_waterfall --har <module>` (which invokes
...
``device_builder`` is the module the CLI entry point imports to start the
server, and importing it pulls in the whole controller tree; a regression here
(for example a fresh eager ``esphome.components.*`` import) lands straight on
dashboard startup, which is painfully slow on low-power hosts like the HA Green.
...
def root_cumulative_us(har: dict[str, Any], module: str) -> int:
    ...
    for entry in _entries(har):
        if entry["request"]["url"] == module:
            return entry["time"]
    raise RuntimeError(
        f"No HAR entry for {module!r}. Is it importable with `python -c 'import {module}'`?"
    )
...
def _print_offenders_table(offenders: list[tuple[str, int, int]], stream: TextIO) -> None:
...
def main() -> int:
    ...
    mode.add_argument("--check", action="store_true", help="Fail if measured time exceeds budget.")
    mode.add_argument(
        "--update",
        action="store_true",
